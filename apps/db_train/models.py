...
class Author(models.Model):
    ...
    bio = models.TextField(verbose_name='Биография',
                            help_text='Напишите тут о том, почему Вы так хороши',
                            null=True,
                            blank=True)
    age = models.IntegerField(null=True, editable=False)
    date_birth = models.DateField(verbose_name='Дата рождения',
                            help_text='Посланцев из будущего мы не регистрируем!',
                            null=True,
                            blank=True)
    ...
    image = models.ImageField(upload_to='foto_profile',
                              ...
                              help_text='Фото в профиль, можно не свое! Ну или хоть какое-то. Ладно, можно без фото!',
                              null=True,
                              blank=True,
                              )
    ...
    def save(self, *args, **kwargs):
        if self.date_birth:
            ...
            self.age = today.year - self.date_birth.year - additional_year
...
class Entry(models.Model):
    ...
    def __str__(self):
        return f'Автор: {self.author.username}; Текст: {self.text}.'
